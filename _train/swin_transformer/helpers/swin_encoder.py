...
from _train.swin_transformer.models.attention import CrossAttention
from _train.swin_transformer.models.swin_transformer import build_model
from _train.swin_transformer.configs.swin_config import get_config
from _train.eg3dc.src.dnnlib.util import EasyDict
...
class GOAEncoder(nn.Module):
    def __init__(self, encoder_config_path, batch_size=4, mlp_layer=2, ws_dim = 14, stage_list=[20000, 40000, 60000]):
        super(GOAEncoder, self).__init__()
        self.style_count = ws_dim       
        self.stage_list = stage_list
        self.stage_dict =   {'base':0, 'coarse':1, 'mid':2, 'fine':3} 
        self.stage = 3
        args = EasyDict(cfg=encoder_config_path, batch_size=batch_size)
