...
# from training.dual_discriminator import filtered_resizingB
...
class MaskDualDiscriminator(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.c_dim = c_dim
        self.img_resolution_h, self.img_resolution_w = img_resolution
        self.img_resolution_log2 = int(np.log2(self.img_resolution_h))
        self.img_channels = img_channels
        self.block_resolutions = [2 ** i for i in range(self.img_resolution_log2, 2, -1)]
        self.seg_resolution_h, self.seg_resolution_w = seg_resolution
        self.seg_resolution_log2 = int(np.log2(self.seg_resolution_h))
        ...
        for res in self.block_resolutions:
            in_channels = channels_dict[res] if res < self.img_resolution_h else 0
            tmp_channels = channels_dict[res]
            out_channels = channels_dict[res // 2]
            use_fp16 = (res >= fp16_resolution)
            block = DiscriminatorBlock(in_channels, tmp_channels, out_channels, resolution=(res,res//2),
                first_layer_idx=cur_layer_idx, use_fp16=use_fp16, **block_kwargs, **common_kwargs)
            setattr(self, f'b{res}', block)
            if res <= self.seg_resolution_h:
                in_channels = channels_dict[res] if res < self.seg_resolution_h else 0
                tmp_channels = channels_dict[res]
                out_channels = channels_dict[res // 2]
                use_fp16 = (res >= fp16_resolution)
                block = DiscriminatorBlock(in_channels, tmp_channels, out_channels, resolution=(res, res//2),
        ...
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=(4,2), **epilogue_kwargs, **common_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
        self.disc_c_noise = disc_c_noise

    def forward(self, img, c, cond, update_emas=False, **block_kwargs):
        image_raw = filtered_resizingB(img['image_raw'], size_h=self.img_resolution_h, size_w=self.img_resolution_w, f=self.resample_filter)
        seg = filtered_resizingB(img['image_mask'], size_h=self.img_resolution_h, size_w=self.img_resolution_w, f=self.resample_filter)
...
def filtered_resizingB(image_orig_tensor, size_h, size_w, f, filter_mode='antialiased'):
    if filter_mode == 'antialiased':
        ada_filtered_64 = torch.nn.functional.interpolate(image_orig_tensor, size=(size_h, size_w), mode='bilinear', align_corners=False, antialias=True)
    elif filter_mode == 'classic':
        ada_filtered_64 = upfirdn2d.upsample2d(image_orig_tensor, f, up=2)
        ada_filtered_64 = torch.nn.functional.interpolate(ada_filtered_64, size=(size_h * 2 + 2, size_w * 2 + 2), mode='bilinear', align_corners=False)
        ada_filtered_64 = upfirdn2d.downsample2d(ada_filtered_64, f, down=2, flip_filter=True, padding=-1)
    elif filter_mode == 'none':
        ada_filtered_64 = torch.nn.functional.interpolate(image_orig_tensor, size=(size_h, size_w), mode='bilinear', align_corners=False)
    elif type(filter_mode) == float:
        assert 0 < filter_mode < 1

        filtered = torch.nn.functional.interpolate(image_orig_tensor, size=(size_h, size_w), mode='bilinear', align_corners=False, antialias=True)
        aliased  = torch.nn.functional.interpolate(image_orig_tensor, size=(size_h, size_w), mode='bilinear', align_corners=False, antialias=False)
...
class DiscriminatorBlock(torch.nn.Module):
    ...
    def extra_repr(self):
        return f'resolution=({self.resolution_h:d},{self.resolution_w:d}), architecture={self.architecture:s}'
...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(self,
        ...
        self.fc = FullyConnectedLayer(in_channels * (self.resolution_h *self.resolution_w), in_channels, activation=activation)
