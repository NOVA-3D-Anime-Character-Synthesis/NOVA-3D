...
class StyleGAN2LossMultiOrthoCond(Loss):
    def __init__(
    ...
    def accumulate_gradients(self, phase, real_img, real_c, real_cond, gen_z, gen_c, gain, cur_nimg):
            ...
            if self.G.cond_mode=='none':
                ...
                if True:
                    ...
                    xin = {
                        ...
                        'distances': distances * torch.ones(len(gen_z), device=gen_z.device),  # forgive me for I have sinned
                        'paste_params': self.paste_params,
                    }
                    out = self.G.f(xin)
        ...
        # Dmain: Minimize logits for generated images.训练鉴别器，让鉴别器能鉴别出生成器生成的图片
        loss_Dgen = 0
