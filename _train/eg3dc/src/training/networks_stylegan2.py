...
from training.encoders.encoder_v0 import Encoder # as Encoder_v0
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        # if self.multi_view_cond_mode != None and encoder_config!=None:
        ...
        for res in self.block_resolutions:
            ...
            self.num_ws += block.num_conv # 1,2*6
            if is_last:
    ...
    def forward(self, ws, cond, latent_injection=None, stop_level=None, return_more=False, **block_kwargs):
        ...
        if self.multi_view_cond_mode == None:
            ...
            # back_feature = self.encoder(cond['image_ortho_back']) # [14,512] -> [64,512] -> [B, 512, 8, 8]
                ...
                for lvl,(res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)): # [4,8,16,32,64,128,256]
                    ...
                    if cond_mode.startswith('ortho_front.'):
                        if (res==8) and (chonkadd > 0): # [batch_gpu, 8 ,512] -> [batch_gpu, 16 ,512]
                            ...
                            # addition or multiplication
                            if 'inj_6b_4' in cm and res==self.block_resolutions[-1]: # [4,8,16,32,64,128,256]
                                # print('YES IT IS WORKING')
                                # toadd = cimg
                                toadd = (cond['image_ortho_front'].flip(dims=(-2,)) *2-1) * 4
                                toadd = torch.nn.functional.interpolate(toadd, size=img.shape[-2:], mode='bilinear')
                        ...
                        if (res==8) and (chonkadd > 0): # [8*8,512]
                            ch = chonkadd # 512
                            x = torch.cat([
                                x[:,:ch] +  cond['resnet_chonk_back'][:,:ch],  # [B, 512, 8, 8]
                                x[:,ch:],
                            ], dim=1)
                        # reshape condition
                        else:
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.img_channels = img_channels
        self.cond_mode = cond_mode
        self.synthesis = SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, cond_mode=cond_mode, multi_view_cond_mode = multi_view_cond_mode,**synthesis_kwargs)
