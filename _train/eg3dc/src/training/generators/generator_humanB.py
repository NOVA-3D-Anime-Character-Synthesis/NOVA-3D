...
from training.encoders.encoder_v0 import Encoder
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        super().__init__()
        self.cond_mode = cond_mode
        self.multi_view_cond_mode = multi_view_cond_mode
        if self.multi_view_cond_mode != None and encoder_kwargs!={}:
            self.encoder = dnnlib.util.construct_class_by_name(class_name=encoder_kwargs['encoder'], encoder_config_path=encoder_kwargs['encoder_config'], batch_size=encoder_kwargs['batch_gpu'])
            # self.cond_mode = None
        else:
            self.encoder=None
        self.w_dim = w_dim
        self.img_resolution = img_resolution # 256
        self.img_resolution_log2 = int(np.log2(img_resolution)) # log2(256)= 7
        self.img_channels = img_channels
    ...
    def forward(self, ws, cond, latent_injection=None, stop_level=None, return_more=False, **block_kwargs):
        ...
        with torch.autograd.profiler.record_function('split_ws'):
            ...
            w_idx = 0
            for res in self.block_resolutions: # [4,8,16,32,64,128,256]
                block = getattr(self, f'b{res}')
                block_ws.append(ws.narrow(1, w_idx, block.num_conv + block.num_torgb)) # 在512中切，     dim:需要切片的维度 start:指定维度开始的位置 length:从开始位置切片的长度
        ...
        if self.multi_view_cond_mode == None:
            cm = set(self.cond_mode.split('.')) #  'ortho_front.cond_img_norm_4.concatfront.crossavg_4.reschonk_add_512.inj_6b_4'
            chonkadd = [int(c.split('_')[-1]) for c in cm if c.startswith('reschonk_add_')]
            chonkadd = 0 if len(chonkadd)==0 else chonkadd[0]
            for lvl,(res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)): # [4,8,16,32,64,128,256]
                block = getattr(self, f'b{res}')
                x, img = block(x, img, cur_ws, **block_kwargs)
                if self.cond_mode=='none':
                    pass
                elif (res==8) and (chonkadd > 0): # [batch_gpu, 8 ,512] -> [batch_gpu, 16 ,512]
                    ch = chonkadd
                    x = torch.cat([
                        x[:,:ch] + cond['resnet_chonk'][:,:ch],
                        x[:,ch:],
                    ], dim=1)
                    # print("x+chonk.shape",x.shape)
                # carry on
                elif self.cond_mode.startswith('ortho_front.'):
            ...
            if stop_level==None: # stop_level=None
                ret = img # [B, 144, 256, 256]
            else:
            ...
            if not return_more: # return_more=False
                return ret
            else:
                return ret, locals() # 返回一个字典
        else: # multi_view
            if self.encoder!=None:
                img_ortho_back = torch.nn.functional.interpolate(cond['image_ortho_back'], size=(self.img_resolution, self.img_resolution), mode='bilinear', align_corners=False, antialias=True)
                back_feature = self.encoder(cond['image_ortho_back']) # B,512,8,8
            else:
                back_feature = cond['resnet_chonk_back']
            # print(back_feature.shape)
            for cond_mode in self.multi_view_cond_mode.split(','):
                cm = set(cond_mode.split('.'))
                chonkadd = [int(c.split('_')[-1]) for c in cm if c.startswith('reschonk_add_')]
                chonkadd = 0 if len(chonkadd)==0 else chonkadd[0]
                # two triplane
                for lvl,(res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)): # [4,8,16,32,64,128,256]
                    block = getattr(self, f'b{res}')
                    x, img = block(x, img, cur_ws, **block_kwargs) # x 是
                    if cond_mode.startswith('ortho_front.'):
                        if (res==8) and (chonkadd > 0): # [batch_gpu, 8 ,512] -> [batch_gpu, 16 ,512]
                            ch = chonkadd
                            x = torch.cat([
                                x[:,:ch] + cond['resnet_chonk_front'][:,:ch],
                                x[:,ch:],
                            ], dim=1)
                        else:
                            cimg = cond['image_ortho_front'].flip(dims=(-2,))
                            ...
                            if 'concatfront' in cm:
                                toadd = cimg
                                toadd = torch.nn.functional.interpolate(toadd, size=x.shape[-2:], mode='bilinear') # (B,C,当前图片的大小,当前图片的大小)
                    ...
                    elif cond_mode.startswith('ortho_back.'):
                        if (res==8) and (chonkadd > 0): # [batch_gpu, 8 ,512] -> [batch_gpu, 16 ,512]
                            ch = chonkadd
                            x = torch.cat([
                                x[:,:ch] + back_feature, #cond['resnet_chonk_back'][:,:ch],
                                x[:,ch:],
                            ], dim=1)
                        # reshape condition
                        else:
                            cimg = cond['image_ortho_back'].flip(dims=(-2,))
                            ...
                            # normalize image
                            cimg = cimg *2-1 # [0,1] * 2 -1 = [-1,1]
                            if 'cond_img_norm_4' in cm: # [-4, 4]
                                cimg = 4 * cimg
                            ...
                            if 'inj_6b_4' in cm and res==self.block_resolutions[-1]:
                                # print('YES IT IS WORKING')
                                # toadd = cimg
                                toadd = (cond['image_ortho_back'].flip(dims=(-2,)) *2-1) * 4
                                toadd = torch.nn.functional.interpolate(toadd, size=img.shape[-2:], mode='bilinear')
                            ...
                            back_img = img
                    # cross average
                    if 'crossavg_4' in cm: # 池化操作用于平滑特征
                        ch = int(x.shape[1] // (4*2))
                        horz = x[:,0:ch]
                        vert = x[:,ch:ch+ch]
                        x = torch.cat([
                ...
                if cond_mode.startswith('ortho_front.') and stop_level==None:
                    front_img = img
                if cond_mode.startswith('ortho_back.') and stop_level==None:
                    back_img = img
                x = img = None
            if not return_more: # return_more=False
                return front_img, back_img
            else:
                # print("return_more = True")
                return front_img, back_img, locals() # 返回一个字典       
                # return ret, locals() # 返回一个字典       
...
class Generator(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality. # 512
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.img_channels = img_channels
        self.cond_mode = cond_mode
        self.synthesis = SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, cond_mode=cond_mode, multi_view_cond_mode = multi_view_cond_mode, encoder_kwargs=encoder_kwargs,**synthesis_kwargs)
