...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        assert img_resolution >= 4 and img_resolution & (img_resolution - 1) == 0 # 必须是2的倍数
        super().__init__()
        self.cond_mode = cond_mode
    ...
    def forward(self, ws, cond, latent_injection=None, stop_level=None, return_more=False, **block_kwargs):
        ...
        if self.multi_view_cond_mode == None:
            ...
            if self.encoder!=None:
                ...
                front_feature = self.encoder(cond['image_ortho_front']) # B,512,8,8
            else:
                ...
                for lvl,(res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)): # [4,8,16,32,64,128,256]
                    ...
                    if cond_mode.startswith('ortho_front.'):
                        if (res==8) and (chonkadd > 0): # [batch_gpu, 8 ,512] -> [batch_gpu, 16 ,512]
                            ch = chonkadd
                            x = torch.cat([
                                x[:,:ch] + front_feature, # cond['resnet_chonk_front'][:,:ch],
                                x[:,ch:],
                            ], dim=1)
                        else:
                            cimg = cond['image_ortho_front'].flip(dims=(-2,))
