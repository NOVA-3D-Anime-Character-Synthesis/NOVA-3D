...
from training.discriminators.discriminator_v0 import MaskDualDiscriminator as MaskDualDiscriminator_humanB
...
class MaskDualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, cond, update_emas=False, **block_kwargs):
        ...
        if self.c_dim > 0:
            if self.disc_c_noise > 0: c += torch.randn_like(c) * c.std(0) * self.disc_c_noise
            cmap = self.mapping(None, c, cond) # 可以使用cond中的特征，但是没有使用这个特征
        x = self.b4(x, img, cmap)
