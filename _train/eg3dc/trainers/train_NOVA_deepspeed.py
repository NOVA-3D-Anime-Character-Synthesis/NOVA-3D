...
import os
import sys
sys.path.append('/HOME/HOME/gameday/gameday-3d-human-reconstruction-multi_view_panic3d-')
# sys.path.append('/HOME/HOME/gameday/gameday-3d-human-reconstruction-multi_view_panic3d-/_train/eg3dc')
...
def launch_training(c, desc, outdir, dry_run):
    ...
    #     else:
    #         torch.multiprocessing.spawn(fn=subprocess_fn, ardugs=(c, temp_dir), nprocs=c.num_gpus)
    
    getattr(training_loop, c.training_loop_version)(**c)
#----------------------------------------------------------------------------
...
@click.option('--multi_view_cond_mode', type=str, required=True)
...
@click.option('--g_num_fp16_res',    help='Number of fp16 layers in generator', metavar='INT', type=click.IntRange(min=0), default=4, required=False, show_default=True)
...
@click.option('--deepspeed_config', type=str, default='./_configs/ds_config.json')
...
def main(**kwargs):
    ...
    c.deepspeed_config = opts.deepspeed_config
    c.local_rank = opts.local_rank
